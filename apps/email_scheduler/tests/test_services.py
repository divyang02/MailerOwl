...
class TestService(TestCase):
        ...
        self.mock_mailjet_return_value = {
            "Messages": [{
                ...
                "To": [{
                    ...
                    1,
                    "MessageHref":
                    "https://api.mailjet.com/v3/REST/message/288230382216082166",
                }],
                "Cc": [],
                "Bcc": [],
            }]
    ...
    def test_send_email_called_with_proper_obj(self):
        with patch(
            ...
            mock_mailjet_email_sender.assert_called_once_with(
                self.email_scheduler)
    ...
    def test_send_email_success(self):
        with patch(
            ...
            email_scheduler_log_created = EmailSchedulerLogs.objects.filter(
                email_scheduler=self.email_scheduler)[0]
            ...
            self.assertEqual(self.email_scheduler.task_status,
        ...
        with patch(
            ...
            mock_mailjet_email_sender.return_value = {
                "Messages": [{
                    ...
                    "Errors": [{
                        "ErrorIdentifier":
                        "21e28b7d-cf57-4178-8481-d9589f84c6c3",
                        "ErrorCode":
                        "send-0003",
                        "StatusCode":
                        400,
            ...
            self.assertEqual(
                ...
                mock_mailjet_email_sender.return_value["Messages"][0]
                ["Errors"],
            )
            self.assertEqual(email_scheduler_err.task_status,
                             TASK_STATUS_FAILED)
    ...
    def test_periodic_email_log_updater(self,
                                        mock_fetch_email_status_by_message_id):
        mock_fetch_email_status_by_message_id.return_value = {
            "Comment":
            "550 5.1.1 The email account that you tried to reach does not exist. Please try 5.1.1 double-checking the recipient's email address for typos or 5.1.1 unnecessary spaces. Learn more at 5.1.1  https://support.google.com/mail/?p=NoSuchUser f3si13307730wmj.221 - gsmtp",
            "EventAt": 1611561599,
